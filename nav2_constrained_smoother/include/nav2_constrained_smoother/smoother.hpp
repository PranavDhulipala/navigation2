...
private:
  ...
  {
    // Create costmap grid
    costmap_grid_ = std::make_shared<ceres::Grid2D<uint8_t>>(
      costmap->getCharMap(), 0, costmap->getSizeInCellsY(), 0, costmap->getSizeInCellsX());
    auto costmap_interpolator = std::make_shared<ceres::BiCubicInterpolator<ceres::Grid2D<uint8_t>>>(
      *costmap_grid_);
  ...
  */
  ...
  std::shared_ptr<ceres::Grid2D<uint8_t>> costmap_grid_;
};
